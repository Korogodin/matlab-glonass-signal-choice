...
fprintf(fid_out, '<html><head><title>Pomex appendix</title> \n');
fprintf(fid_out, '<meta charset="utf-8"> </head> <body> \n');
for f_in = 1:1
    % 81 хорошо делится на 3, 80 на 2
%    1 2 3
%    4 5 6
    sum_dB_sin = Pomexoyst_BoCsin(:, :);
    sum_dB_cos = Pomexoyst_BoCcos(:, :);
    for xm = 1:max_xm
        for xn = 1:max_xn
            fprintf(fid_out, '<b>Table C.%.0f</b>. <i>The separation factor between band noise and BOC<sub>sin</sub>(m, n) (blue) and BOC<sub>cos</sub>(m, n) (pink) signals, dB</i><br> \n', pril_num);
            pril_num = pril_num + 1;
...
% Теперь для BPSK
sum_dB_BPSK = Pomexoyst_BPSK(:);
max_n_on_row = 20;
xn_max = 4;

fprintf(fid_out, '<b>Table C.%.0f</b>. <i>The separation factor between band noise and BPSK(n) signals, dB</i><br> \n', pril_num);
pril_num = pril_num + 1;
fprintf(fid_out, '<table width="1000" cellpadding="0" cellspacing="0" border="0" bgcolor="#000000"><tr><td>\n'); 
fprintf(fid_out, '<TABLE width="100%%" cellpadding="0" cellspacing="1" border="0" bgcolor="#000000">\n');
fprintf(fid_out, '<FONT SIZE="1">\n');
