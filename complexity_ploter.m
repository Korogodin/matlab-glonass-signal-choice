...
if Signal_Type == BOCsin
    ...
    for f_in = 1:fmax
        ...
        if DoPlot
            ...
            title(sprintf('Complexity for BOC_{sin} L1 signals at f_n = %.0f, scores', farr(f_in)), 'FontSize', 10);
...
elseif Signal_Type == BOCcos
    ...
    for f_in = 1:fmax
        ...
        if DoPlot
            ...
            set(gca, 'FontSize', 10)
            set(hF, 'PaperPosition', [0 0 14 11]);   
            title(sprintf('Complexity for BOC_{cos} L1 signals at f_n = %.0f, scores', farr(f_in)), 'FontSize', 10);
...
elseif Signal_Type == BPSK
    ...
    if DoPlot
        ...
        set(gca, 'FontSize', 10)
        set(hF, 'PaperPosition', [0 0 14 11]);   
        title(sprintf('Complexity for BPSK L1 signals, scores'), 'FontSize', 10);
