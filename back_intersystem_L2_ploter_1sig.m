    ...
    for f_in = 1:fmax
        ...
        sum_dB = sum_dB + NaNder(:, :, f_in);
            if DoPlot
                ...
                title(sprintf('k_{sd} between BOC_{sin} and received GPS L2 %s, f_n = %.0f, dB', signal_str, farr(f_in)), 'FontSize', 10);
    ...
    for f_in = 1:fmax
        ...
        sum_dB = sum_dB + NaNder(:, :, f_in);
            if DoPlot
                ...
                set(gca, 'FontSize', 10)
                set(hF, 'PaperPosition', [0 0 14 11]);    
                title(sprintf('k_{sd} between BOC_{cos} and received GPS L2 %s, f_n = %.0f, dB', signal_str, farr(f_in)), 'FontSize', 10);
...
elseif Signal_Type == BPSK
    ...
    if DoPlot
        ...
        set(gca, 'FontSize', 10)
        set(hF, 'PaperPosition', [0 0 14 11]);             
        title(sprintf('k_{sd} between BPSK and received GPS L2 %s, dB', signal_str), 'FontSize', 10);
