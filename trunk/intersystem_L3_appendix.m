...
path_to_results = [pwd '/results/intersystem_L3/Common'];

load([path_to_results '/InterSysJam_BoCsin_L3.mat'], 'InterSysJam_BoCsin_L3');
load([path_to_results '/InterSysJam_BoCcos_L3.mat'], 'InterSysJam_BoCcos_L3');
load([path_to_results '/InterSysJam_BPSK_L3.mat'], 'InterSysJam_BPSK_L3');
...
fprintf(fid_out, '<html><head><title>InterNam L3 appendix</title> \n');
...
%    пределы полосы
    sum_dB_sin = InterSysJam_BoCsin_L3(:, :, f_in);
    sum_dB_cos = InterSysJam_BoCcos_L3(:, :, f_in);
    freq_index = f_in;
    for xm = 1:max_xm
        for xn = 1:max_xn
            ...
            if ( min_sum_m8_and_n8/8 > (freq_index + 2) ) || ( min_sum_m8_and_n8/8 > ((20-freq_index) +2) ) % Полоса
                continue;
            end
                fprintf(fid_out, '<b>Table G.%.0f</b>. <i>Сoefficient k<sub>intersys</sub> for BOC<sub>sin</sub>(m, n) (blue) and BOC<sub>cos</sub>(m, n) (pink) L3 signals with f<sub>n</sub>=%.0f when receiving signals of selectively set, dB</i><br> \n', pril_num, farr(f_in));
                pril_num = pril_num + 1;
...
% Теперь для BPSK
sum_dB_BPSK = InterSysJam_BPSK_L3(:, :);
max_n_on_page = 40;
max_xn = 2;
...
%    1 
%    2 
    fprintf(fid_out, '<b>Table G.%.0f</b>. <i>Сoefficient k<sub>intersys</sub> for BPSK(n) L3 signals when receiving signals of selectively set, dB</i><br> \n', pril_num);
    pril_num = pril_num + 1;
    fprintf(fid_out, '<table width="1000" cellpadding="0" cellspacing="0" border="0" bgcolor="#000000"><tr><td>\n'); 
    fprintf(fid_out, '<TABLE width="100%%" cellpadding="0" cellspacing="1" border="0" bgcolor="#000000">\n');
    fprintf(fid_out, '<FONT SIZE="1">\n');
