...
% 4 - M-code
pic_num = 529;
pic_num = pic_num + 33*(Out_Type-1-1*(Out_Type==4));
if Out_Type == 1
    filename_pref = 'CA_';
    signal_str = 'C/A';
elseif Out_Type == 2
...
for Signal_Type = 1:3
    for f_in = 1:fmax
        if (Signal_Type == BOCsin) 
            ...
            fprintf('<b>Рисунок 1.7.%.0f</b> -  <i>Separation factor between received BoC<sub>sin</sub>(m, n) signals<br>and GPS L2 %s signals at f<sub>n</sub>=%.0f</i><br><br> \n', pic_num, signal_str, farr(f_in));
        elseif (Signal_Type == BOCcos)
            fprintf('<img src="%s" /><br> \n', ['png/' filename_pref 'k_sd_BoCcos_at_' sprintf('%3.0f', farr(f_in)) '.png']);
            fprintf('<b>Рисунок 1.7.%.0f</b> -  <i>Separation factor between received BoC<sub>cos</sub>(m, n) signals<br>and GPS L2 %s signals at f<sub>n</sub>=%.0f</i><br><br> \n', pic_num, signal_str, farr(f_in));
        elseif (Signal_Type == BPSK)
            fprintf('<img src="%s" /><br> \n', ['png/' filename_pref 'k_sd_BPSK.png']);
            fprintf('<b>Рисунок 1.7.%.0f</b> -  <i>Separation factor between received BPSK(n) signals<br>and GPS L2 %s signals</i><br><br> \n', pic_num, signal_str);
        end
        pic_num = pic_num + 1;
        if Signal_Type == BPSK
            break;
