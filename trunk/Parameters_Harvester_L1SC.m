...
size_BOCsin = size(Signals_BOCsin_L1SC, 1)*(~isnan(Signals_BOCsin_L1SC(1,1)));
size_BOCcos = size(Signals_BOCcos_L1SC, 1)*(~isnan(Signals_BOCcos_L1SC(1,1)));
size_BPSK = size(Signals_BPSK_L1SC, 1)*(~isnan(Signals_BPSK_L1SC(1,1)));
...
All_Signals_index = 0;
    ...
    elseif Table_Type == 2
        if TopSize < All_Signals_Size
            fprintf('|+ Multi-objective optimization for L1SC signal (Top %.0f)\n', TopSize);
        else
