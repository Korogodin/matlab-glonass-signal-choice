...
load([path_to_results '/BackInterSysJam_BoCcos_L1.mat'], 'BackInterSysJam_BoCcos_L1');
load([path_to_results '/BackInterSysJam_BPSK_L1.mat'], 'BackInterSysJam_BPSK_L1');
...
fprintf(fid_out, '<html><head><title>InterIm L1 appendix</title> \n');
...
%    пределы полосы
    sum_dB_sin = BackInterSysJam_BoCsin_L1(:, :, f_in);
    sum_dB_cos = BackInterSysJam_BoCcos_L1(:, :, f_in);
    freq_index = f_in;
    for xm = 1:max_xm
        for xn = 1:max_xn
            ...
            if ( min_sum_m8_and_n8/8 > (freq_index + 2) ) || ( min_sum_m8_and_n8/8 > ((16-freq_index) +2) ) % Полоса
                ...
                fprintf(fid_out, '<b>Table H.%.0f</b>. <i>Mean separation factor for BOC<sub>sin</sub>(m, n) (blue) and BOC<sub>cos</sub>(m, n) (pink) L1 signals with f<sub>n</sub>=%.0f when receiving other systems signals, dB</i><br> \n', pril_num, farr(f_in));
                pril_num = pril_num + 1;
...
% Теперь для BPSK
sum_dB_BPSK = BackInterSysJam_BPSK_L1(:, :);
max_n_on_page = 40;
max_xn = 2;
...
%    1 
%    2 
    fprintf(fid_out, '<b>Table H.%.0f</b>. <i>Mean separation factor for BPSK(n) L1 signals  when receiving other systems signals, dB</i><br> \n', pril_num);
    pril_num = pril_num + 1;
    fprintf(fid_out, '<table width="1000" cellpadding="0" cellspacing="0" border="0" bgcolor="#000000"><tr><td>\n'); 
    fprintf(fid_out, '<TABLE width="100%%" cellpadding="0" cellspacing="1" border="0" bgcolor="#000000">\n');
    fprintf(fid_out, '<FONT SIZE="1">\n');
