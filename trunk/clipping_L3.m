...
path_to_results_compl = [pwd '/results/complexity_L3'];
...
path_to_results_back_inter_C = [pwd '/results/back_intersystem_L3/Common'];
load([path_to_results_back_inter_C '/BackInterSysJam_BoCsin_L3.mat'], 'BackInterSysJam_BoCsin_L3');
load([path_to_results_back_inter_C '/BackInterSysJam_BoCcos_L3.mat'], 'BackInterSysJam_BoCcos_L3');
load([path_to_results_back_inter_C '/BackInterSysJam_BPSK_L3.mat'], 'BackInterSysJam_BPSK_L3');
...
for Access_Type = 2:2
    ...
    if Access_Type == OpenSignal
        fprintf('|+ L3 open signals after clipping \n');
        Threshold_Pomex = Pomexoyst_BPSK(8*8);
        Threshold_Accur = 1/Beta_BPSK(8*8);
        Threshold_Intra = InSysJam_BPSK(8*8);
        Threshold_Complexity = Complexity_BPSK(10*8, 12);
    %     Threshold_Search = max(Best_Search);
    elseif Access_Type == CloseSignal
        fprintf('|+ L3 special Signals after clipping \n');
        Threshold_Pomex = Pomexoyst_BPSK(8*8);
        Threshold_Accur = 1/Beta_BPSK(8*8);
        Threshold_Intra = InSysJam_BPSK(8*8);
        Threshold_Complexity = Complexity_BPSK(10*8, 12);
    ...
    for Signal_Type = 1:3
        ...
        if Access_Type == OpenSignal
            ...
            if Signal_Type == BOCsin
                fid_BOCsin = fopen('stuff/complexity_stuff/L3OC_BOCsin.txt', 'w');
            elseif Signal_Type == BOCcos
                fid_BOCcos = fopen('stuff/complexity_stuff/L3OC_BOCcos.txt', 'w');
            elseif Signal_Type == BPSK
                fid_BPSK = fopen('stuff/complexity_stuff/L3OC_BPSK.txt', 'w');
            end
        elseif Access_Type == CloseSignal
            max_HalfBand = 5 + 2.5 + 100;
            if Signal_Type == BOCsin
                fid_BOCsin = fopen('stuff/complexity_stuff/L3SC_BOCsin.txt', 'w');
            elseif Signal_Type == BOCcos
                fid_BOCcos = fopen('stuff/complexity_stuff/L3SC_BOCcos.txt', 'w');
            elseif Signal_Type == BPSK
                fid_BPSK = fopen('stuff/complexity_stuff/L3SC_BPSK.txt', 'w');
            end
        end

        Signals_BOCsin_L3OC = [NaN NaN NaN];
        Signals_BOCcos_L3OC = [NaN NaN NaN];
        Signals_BPSK_L3OC = [NaN NaN];
        Signals_BOCsin_L3SC = [NaN NaN NaN];
        Signals_BOCcos_L3SC = [NaN NaN NaN];
        Signals_BPSK_L3SC = [NaN NaN];
                                    ...
                                    if Complexity_BoCsin(m8, n8, freq_index) <= Threshold_Complexity
                                        if ((m+n + farr(freq_index)) <= (farr(end) + 1)) && ((farr(freq_index) - (m+n)) >= (farr(1) - 1))...
                                            ...
                                            if (Access_Type == OpenSignal)||(n == 2.5)||(n == 2.25)||(n == 2.625)||(n == n)
                                                ...
                                                if WideTable
                                                    fprintf('|| %.1f ', BackInterSysJam_BoCsin_L3(m8, n8, freq_index));
                                                    fprintf('|| %.1f ', InterSysJam_BoCsin_L3(m8, n8, freq_index));
                                                ...
                                                if Access_Type == OpenSignal
                                                    Signals_BOCsin_L3OC(counter, 1) = m;
                                                    Signals_BOCsin_L3OC(counter, 2) = n;
                                                    Signals_BOCsin_L3OC(counter, 3) = farr(freq_index);
                                                elseif Access_Type == CloseSignal
                                                    Signals_BOCsin_L3SC(counter, 1) = m;
                                                    Signals_BOCsin_L3SC(counter, 2) = n;
                                                    Signals_BOCsin_L3SC(counter, 3) = farr(freq_index);
                                                end
                                            end
                                        end
                                    end
                                    ...
                                    if Complexity_BoCcos(m8, n8, freq_index) <= Threshold_Complexity
                                        if ((m+n + farr(freq_index)) <= (farr(end) + 1)) && ((farr(freq_index) - (m+n)) >= (farr(1) - 1))...
                                            ...
                                            if (Access_Type == OpenSignal)||(n == 2.5)||(n == 2.25)||(n == 2.625)||(n == n)
                                                ...
                                                if WideTable
                                                    fprintf('|| %.1f ', BackInterSysJam_BoCcos_L3(m8, n8, freq_index));
                                                    fprintf('|| %.1f ', InterSysJam_BoCcos_L3(m8, n8, freq_index));
                                                ...
                                                if Access_Type == OpenSignal
                                                    Signals_BOCcos_L3OC(counter, 1) = m;
                                                    Signals_BOCcos_L3OC(counter, 2) = n;
                                                    Signals_BOCcos_L3OC(counter, 3) = farr(freq_index);
                                                elseif Access_Type == CloseSignal
                                                    Signals_BOCcos_L3SC(counter, 1) = m;
                                                    Signals_BOCcos_L3SC(counter, 2) = n;
                                                    Signals_BOCcos_L3SC(counter, 3) = farr(freq_index);
                                                end                                        
                                            end
                                        end
                                    end
                                    ...
                                    if Complexity_BPSK(n8, freq_index) <= Threshold_Complexity
                                        if ((m+n + farr(freq_index)) <= (farr(end) + 1)) && ((farr(freq_index) - (m+n)) >= (farr(1) - 1))...
                                            ...
                                            if (Access_Type == OpenSignal)||(n == 2.5)||(n == 2.25)||(n == 2.625)||(n == n)
                                                ...
                                                if WideTable
                                                    fprintf('|| %.1f ', BackInterSysJam_BPSK_L3(n8, freq_index));
                                                    fprintf('|| %.1f ', InterSysJam_BPSK_L3(n8, freq_index));
                                                ...
                                                fprintf(fid_BPSK, '%.3f \n', n8/8);
                                                if Access_Type == OpenSignal
                                                    Signals_BPSK_L3OC(counter, 1) = m;
                                                    Signals_BPSK_L3OC(counter, 2) = n;
                                                    Signals_BPSK_L3OC(counter, 3) = farr(freq_index);
                                                elseif Access_Type == CloseSignal
                                                    Signals_BPSK_L3SC(counter, 1) = m;
                                                    Signals_BPSK_L3SC(counter, 2) = n;
                                                    Signals_BPSK_L3SC(counter, 3) = farr(freq_index);
                                                end                                        
                                            end
                                        end
                                    end
        ...
        if Signal_Type == BOCsin
            if Access_Type == OpenSignal
                save('results/clipping/Signals_BOCsin_L3OC.mat', 'Signals_BOCsin_L3OC');
            elseif Access_Type == CloseSignal
                save('results/clipping/Signals_BOCsin_L3SC.mat', 'Signals_BOCsin_L3SC');
            end
        elseif Signal_Type == BOCcos
            if Access_Type == OpenSignal
                save('results/clipping/Signals_BOCcos_L3OC.mat', 'Signals_BOCcos_L3OC');
            elseif Access_Type == CloseSignal
                save('results/clipping/Signals_BOCcos_L3SC.mat', 'Signals_BOCcos_L3SC');
            end
        elseif Signal_Type == BPSK
            if Access_Type == OpenSignal
                save('results/clipping/Signals_BPSK_L3OC.mat', 'Signals_BPSK_L3OC')
            elseif Access_Type == CloseSignal
                save('results/clipping/Signals_BPSK_L3SC.mat', 'Signals_BPSK_L3SC')
            end
        end
