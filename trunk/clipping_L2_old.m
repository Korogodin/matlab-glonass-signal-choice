        ...
        for n8 = 1:80
            for m8 = 1:80
                ...
                for freq_index = 1:fmax
                    if Signal_Type == BOCsin
        %                 ploxoj = 0;
        %                 for jj = 1:length(Best_Accur)
        %                     if (Pomexoyst_BoCsin(m8, n8) >= Best_Pomex(jj)) && ...
        %                             ((1/Beta_BoCsin(m8, n8)) >= Best_Accur(jj)) && ...
        %                                 (InSysJam_BoCsin(m8, n8) >= Best_Intra(jj)) && ...
        %                                     (Complexity_BoCsin(m8, n8, freq_index) >= Best_Compl(jj))
        %                                 ploxoj = 1;
        %                     end
        %                 end
        %                 if ploxoj
                                ...
                                if InSysJam_BoCsin(m8, n8) <= Threshold_Intra
                                    if Complexity_BoCsin(m8, n8, freq_index) <= Threshold_Complexity
                                        if ((m+n + farr(freq_index)) <= (farr(end) + 1)) && ((farr(freq_index) - (m+n)) >= (farr(1) - 1))...
                                                && ((m+n) <= max_HalfBand)
                                            if (Access_Type == OpenSignal)||(n == 2.5)||(n == 2.25)||(n == 2.625)
                                                fprintf('\n|- align="center"\n');
